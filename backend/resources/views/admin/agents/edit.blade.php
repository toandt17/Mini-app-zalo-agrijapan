    ...
    <div class="main-container container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header">
                <div>
                    <h2 class="main-content-title tx-24 mg-b-5">Chỉnh sửa đại lý</h2>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:;">Quản lý đại lý</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Chỉnh sửa đại lý</li>
                    </ol>
                </div>
                <div class="d-flex">
                ...
                <div class="col-xl-12 col-lg-12 col-md-12">
                    <div class="card custom-card">
                        <div class="card-header">
                            <div>
                                <h6 class="main-content-label tx-15">Chỉnh sửa thông tin đại lý</h6>
                            </div>
                        </div>
                        <div class="card-body">
                            <form class="row g-3 needs-validation" action="{{ route('admin.agents.update', $agent->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                                @csrf
                                @method('PUT')
                                <div class="col-md-6 position-relative">
                                    <label for="name" class="form-label tx-semibold">Tên đại lý <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $agent->name) }}" placeholder="Nhập tên đại lý" required>
                                    @error('name')
                                ...
                                <div class="col-md-6 position-relative">
                                    ...
                                    <input type="text" class="form-control" id="phone" name="phone" value="{{ old('phone', $agent->phone) }}" placeholder="Nhập số điện thoại" required>
                                    @error('phone')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-12 position-relative">
                                    ...
                                    <select class="form-control" id="status" name="status">
                                        <option value="active" {{ old('status', $agent->status) == 'active' ? 'selected' : '' }}>Hoạt động</option>
                                        <option value="inactive" {{ old('status', $agent->status) == 'inactive' ? 'selected' : '' }}>Không hoạt động</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4 position-relative">
                                    ...
                                    <select class="form-control" id="province_id" name="province_id" required onchange="loadDistricts()">
                                        ...
                                        @foreach($provinces as $province)
                                            <option value="{{ $province->id }}" {{ old('province_id', $agent->province_id) == $province->id ? 'selected' : '' }}>
                                                {{ $province->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12 position-relative">
                                    ...
                                    <input type="text" class="form-control" id="address" name="address" value="{{ old('address', $agent->address) }}" placeholder="Số nhà, tên đường..." required>
                                    @error('address')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 position-relative">
                                    ...
                                    <input type="text" class="form-control" id="latitude" name="latitude" value="{{ old('latitude', $agent->latitude) }}" placeholder="Nhập vĩ độ">
                                    @error('latitude')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 position-relative">
                                    ...
                                    <input type="text" class="form-control" id="longitude" name="longitude" value="{{ old('longitude', $agent->longitude) }}" placeholder="Nhập kinh độ">
                                    @error('longitude')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
        ...
        if (provinceId) {
            fetch(`/districts/${provinceId}`)
                ...
                .then(data => {
                    ...
                    // Nếu đang ở chế độ edit, chọn quận/huyện đã lưu
                    var savedDistrictId = "{{ old('district_id', $agent->district_id) }}";
                    if (savedDistrictId) {
        ...
        if (districtId) {
            fetch(`/wards/${districtId}`)
                ...
                .then(data => {
                    ...
                    // Nếu đang ở chế độ edit, chọn phường/xã đã lưu
                    var savedWardId = "{{ old('ward_id', $agent->ward_id) }}";
                    if (savedWardId) {
                        document.getElementById('ward_id').value = savedWardId;
                    }
                })
